...
const WishListTextFields = ({
	...
	const validateTotalAmount = (totalAmount) => {
		if (totalAmount === '') {
			...
			setErrors({ ...errors, totalAmountError: '' })
			return
		}
	}
	...
	const validateSavingsAmount = (savingsAmount) => {
		if (savingsAmount === '') {
			...
			setErrors({
				...errors,
				savingsAmountError:
					'Savings Amount cannot be less than or equal to 0!'
			})
			return
		} else {
			setErrors({ ...errors, savingsAmountError: '' })
			return
		}
	}
	...
	const validateCurrentAmount = (currentAmount) => {
		if (currentAmount === '') {
			...
			setErrors({
				...errors,
				currentAmountError:
					'Current Amount cannot be less than or equal to 0!'
			})
			return
		} else {
			setErrors({ ...errors, currentAmountError: '' })
			return
		}
	}
		...
		<>
			...
			<Box mt={2}>
				<TextField
					...
					onChange={(event) => {
						...
						validateTotalAmount(event.target.value)
					}}
			...
			<Box mt={2}>
				<FormControl
					variant='outlined'
					fullWidth
					error={errors.savingsAmountError !== ''}
				>
					<InputLabel htmlFor='outlined-adornment-savings-amount'>
						Savings Amount
					</InputLabel>
					<OutlinedInput
						...
						onChange={(event) => {
							...
							validateSavingsAmount(event.target.value)
						}}
					/>
				</FormControl>
				<FormHelperText
					id='outlined-adornment-savings-amount'
					style={{ color: 'red' }}
				>
					{errors.savingsAmountError}
				</FormHelperText>
				...
				<Box mt={2}>
					<TextField
						...
						onChange={(event) => {
							...
							validateCurrentAmount(event.target.value)
						}}
